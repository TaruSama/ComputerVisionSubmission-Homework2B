...
class CaptioningTransformer(nn.Module):
    ...
    def __init__(self, word_to_idx, input_dim, wordvec_dim, num_heads=4,
        ...
        # aliases expected by some reference/older code
        self.caption_embedding = self.embedding
        self.feature_projection = self.visual_projection
        self.decoder = self.transformer
        self.output_projection = self.output
    ...
    def forward(self, features, captions):
        ...
        # 2) Image features -> memory sequencev
        memory = self.visual_projection(features)   # (N, W)
        memory = memory.unsqueeze(1)                # (N, 1, W)
    
        # 3) Causal mask: in this codebase, attn_mask[i, j] == 0 means "block"
        # token i from influencing token j. So we want LOWER-triangular ones.
        tgt_mask = torch.tril(
            torch.ones(T, T, device=captions.device, dtype=torch.int64)
        )
    
        # 4) Decode + vocab projection
        out = self.transformer(tgt=tgt, memory=memory, tgt_mask=tgt_mask)  # (N, T, W)
...
class VisionTransformer(nn.Module):
    ...
    def forward(self, x):
        ...
        patches = self.positional_encoding(patches)   # (N, num_patches, embed_dim)
    
        # 3) Transformer encoder (no mask needed)
        feats = self.transformer(patches, src_mask=None)  # (N, num_patches, embed_dim)
    
        # 4) Average pool over patches
        pooled = torch.mean(feats, dim=1)             # (N, embed_dim)
