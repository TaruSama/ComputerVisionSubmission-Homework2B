...
def clip_zero_shot_classifier(clip_model, clip_preprocess, images,
    ...
    image_inputs = torch.stack([
        clip_preprocess(Image.fromarray(img)).to(device)
        for img in images
    ])
    ...
    pred_classes = [class_texts[i] for i in preds.tolist()]
    ############################################################################
    #                             END OF YOUR CODE                             #
    ############################################################################
...
class CLIPImageRetriever:
    ...
    def __init__(self, clip_model, clip_preprocess, images, device):
        ...
        # preprocess and encode all images once
        image_inputs = torch.stack([
    ...
    def retrieve(self, query: str, k: int = 2):
        ...
        # cosine similarity
        similarity = self.image_features @ text_features.T  # (N, 1)

        # top-k indices
        top_indices = similarity.squeeze(1).topk(k).indices.tolist()
...
class DINOSegmentation:
    def __init__(self, device, num_classes: int, inp_dim : int = 384):
        ...
        # very lightweight model: linear classifier
