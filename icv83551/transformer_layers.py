...
class MultiHeadAttention(nn.Module):
    ...
    def forward(self, query, key, value, attn_mask=None):
        ...
        # Project + split heads: (N, *, E) -> (N, H, *, head_dim)
        key, value, query = [
            layer(x).view(N, -1, self.n_head, self.head_dim).transpose(1, 2)
            for layer, x in zip((self.key, self.value, self.query), (key, value, query))
        ]
    
        # (N, H, S, head_dim) x (N, H, head_dim, T) -> (N, H, S, T)
        scores = torch.matmul(query, key.transpose(-2, -1)) / math.sqrt(self.head_dim)
    
        if attn_mask is not None:
            # IMPORTANT: mask convention here is "0/False = BLOCK", "1/True = ALLOW"
            # So we mask-fill where mask == 0 (works for both bool and numeric masks).
            scores = scores.masked_fill(attn_mask == 0, -1e12)
    
        attn = F.softmax(scores, dim=-1)
        attn = self.attn_drop(attn)
    
        # (N, H, S, T) x (N, H, T, head_dim) -> (N, H, S, head_dim)
        output = torch.matmul(attn, value)
    
        # Merge heads: (N, H, S, head_dim) -> (N, S, E)
...
class PatchEmbedding(nn.Module):
    ...
    def forward(self, x):
        ...
        # (N, C, H/P, P, W/P, P) -> (N, H/P, W/P, C, P, P)
        x = x.permute(0, 2, 4, 1, 3, 5)
    
        # (N, H/P, W/P, C, P, P) -> (N, num_patches, patch_dim)
        patches = x.reshape(N, self.num_patches, self.patch_dim)
    
        # Project to embedding dim: (N, num_patches, embed_dim)
